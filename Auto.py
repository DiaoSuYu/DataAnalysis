...
def main():
    ...
    # 输入窗口
    root.title('51job岗位数据查询')  # 窗口标题
    Label(root, text='请输入岗位数据关键词').grid(row=0, column=0)  # 设置标签并调整位置
    Label(root, text='请输入查询页数').grid(row=1, column=0)  # 设置标签并调整位置
    enter1 = Entry(root)  # 输入框
    enter1.grid(row=0, column=1, padx=20, pady=20)  # 调整位置
    enter1.delete(0, END)  # 清空输入框
    enter1.insert(0, '51job')  # 设置默认文本

    enter2 = Entry(root)  # 输入框
    enter2.grid(row=1, column=1, padx=20, pady=20)  # 调整位置
    enter2.delete(0, END)  # 清空输入框
    enter2.insert(0, '0')  # 设置默认文本
    ...
    def test():
        city_name = enter1.get()  # 获取输入框的内容
        page_name = enter2.get()  # 获取输入框的内容
        keyword_raw = city_name
        pages = page_name
    ...
    # 布置按键
    Button(root, text="确认", width=10, command=test).grid(row=3, column=0, sticky=W, padx=10, pady=5)
